...
import { Loader2, Trophy, BrainCircuit, Play, MousePointerClick, Eye, Terminal, Monitor, Target, Lightbulb } from 'lucide-react';
...
const GeminiSlingshot: React.FC = () => {
  ...
  const numColsRef = useRef<number>(12); 
  ...
  const getBubblePos = (row: number, col: number) => {
    const isOdd = row % 2 !== 0;
    const x = BUBBLE_RADIUS + col * (BUBBLE_RADIUS * 2) + (isOdd ? BUBBLE_RADIUS : 0);
  ...
  const checkFloating = () => {
    ...
    while (head < queue.length) {
      ...
      const neighbors = activeBubbles.filter(n => !connectedToTop.has(n.id) && isNeighbor(curr, n));
      neighbors.forEach(n => {
    ...
    if (floating.length > 0) {
      ...
      floating.forEach(b => {
        ...
        extraPoints += COLOR_CONFIG[b.color].points;
      });
      scoreRef.current += extraPoints;
  ...
  const checkMatches = (startBubble: Bubble) => {
    ...
    if (matches.length >= 3) {
      ...
      scoreRef.current += Math.floor(points * (matches.length > 3 ? 1.5 : 1.0));
      setScore(scoreRef.current);
      checkFloating();
      return true;
    }
  ...
  const initGrid = useCallback((width: number) => {
    ...
    const numCols = Math.floor((width - BUBBLE_RADIUS) / (BUBBLE_RADIUS * 2));
    numColsRef.current = numCols;

    for (let r = 0; r < 6; r++) { 
      const maxC = r % 2 !== 0 ? numCols - 1 : numCols;
        ...
        for (const b of activeBubbles) {
            ...
            if (hittableMember) {
                ...
                allClusters.push({
                    ...
                    description: xPct < 0.33 ? "Left" : xPct > 0.66 ? "Right" : "Center"
                });
            }
        }
    }
  ...
  useEffect(() => {
    ...
    const onResults = (results: any) => {
      ...
      if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
        ...
        handPos = {
          x: (landmarks[8].x * canvas.width + landmarks[4].x * canvas.width) / 2,
          y: (landmarks[8].y * canvas.height + landmarks[4].y * canvas.height) / 2
        };
        const dx = landmarks[8].x - landmarks[4].x, dy = landmarks[8].y - landmarks[4].y;
      ...
      if (isFlying.current) {
        if (performance.now() - flightStartTime.current > 5000) {
            ...
            const steps = Math.ceil(speed / (BUBBLE_RADIUS * 0.8)); 
            ...
            for (let i = 0; i < steps; i++) {
                ...
                if (ballPos.current.x < BUBBLE_RADIUS || ballPos.current.x > canvas.width - BUBBLE_RADIUS) {
                    ballVel.current.x *= -1; ballPos.current.x = Math.max(BUBBLE_RADIUS, Math.min(canvas.width - BUBBLE_RADIUS, ballPos.current.x));
                }
                if (ballPos.current.y < BUBBLE_RADIUS) { collisionOccurred = true; break; }
                for (const b of bubbles.current) {
                    if (!b.active) continue;
                    const d = Math.sqrt(Math.pow(ballPos.current.x - b.x, 2) + Math.pow(ballPos.current.y - b.y, 2));
                    if (d < BUBBLE_RADIUS * 1.8) { collisionOccurred = true; break; }
                }
            ...
            if (collisionOccurred) {
                ...
                for (let r = 0; r < 17; r++) {
                    ...
                    const colsInRow = r % 2 !== 0 ? numCols - 1 : numCols;
                ...
                updateAvailableColors();
                ballPos.current = { ...anchorPos.current }; ballVel.current = { x: 0, y: 0 }; captureRequestRef.current = true;
            }
      ...
      // Draw Dotted Aiming Line when pulling back
      if (isPinching.current) {
        const aimDx = anchorPos.current.x - ballPos.current.x;
        const aimDy = anchorPos.current.y - ballPos.current.y;
        ...
        if (aimDist > 20) {
            ...
            // Draw line starting from ball moving forward
            ctx.moveTo(ballPos.current.x, ballPos.current.y);
            ctx.lineTo(ballPos.current.x + normX * 800, ballPos.current.y + normY * 800);
            ctx.stroke();
            ctx.restore();
        }
      }
      ...
      if (captureRequestRef.current) {
        ...
        const offscreen = document.createElement('canvas'); const scale = Math.min(1, 480 / canvas.width);
  ...
  const borderColor = aiRecommendedColor ? COLOR_CONFIG[aiRecommendedColor].hex : '#444746';
  ...
  return (
    <div className="flex w-full h-screen bg-[#121212] overflow-hidden font-roboto text-[#e3e3e3]">
      <div className="fixed inset-0 z-[100] bg-[#121212] flex flex-col items-center justify-center p-8 text-center md:hidden">
         ...
         <p className="text-[#c4c7c5] max-w-md text-lg leading-relaxed">Large screen required for webcam tracking.</p>
      </div>
      ...
      <div ref={gameContainerRef} className="flex-1 relative h-full overflow-hidden">
        ...
        <div className="absolute top-6 left-6 z-40">
            <div className="bg-[#1e1e1e] p-5 rounded-[28px] border border-[#444746] shadow-2xl flex items-center gap-4 min-w-[180px]">
                ...
                <div><p className="text-xs text-[#c4c7c5] uppercase tracking-wider font-medium">Score</p><p className="text-3xl font-bold text-white">{score.toLocaleString()}</p></div>
            </div>
        </div>
        ...
        {!isPinching.current && !isFlying.current && !isAiThinking && (
            <div className="absolute bottom-28 left-1/2 -translate-x-1/2 z-30 pointer-events-none opacity-50">
                <div className="flex items-center gap-2 bg-[#1e1e1e]/90 px-4 py-2 rounded-full border border-[#444746] backdrop-blur-sm"><Play className="w-3 h-3 text-[#42a5f5] fill-current" /><p className="text-[#e3e3e3] text-xs font-medium">Pinch & Pull to Shoot</p></div>
            </div>
        )}
      </div>
      ...
      <div className="w-[380px] bg-[#1e1e1e] border-l border-[#444746] flex flex-col h-full overflow-hidden shadow-2xl">
        <div className="p-5 border-b-4 transition-colors duration-500 flex flex-col gap-2" style={{ backgroundColor: '#252525', borderColor: borderColor }}>
             <div className="flex items-center justify-between"><div className="flex items-center gap-2"><BrainCircuit className="w-5 h-5" style={{ color: borderColor }} /><h2 className="font-bold text-sm tracking-widest uppercase" style={{ color: borderColor }}>Flash Strategy</h2></div>{isAiThinking && <Loader2 className="w-4 h-4 animate-spin text-white/50" />}</div>
        ...
        <div className="p-3 border-b border-[#444746] bg-[#1e1e1e] flex items-center gap-2 text-[#757575]"><Terminal className="w-4 h-4" /><span className="text-[10px] font-bold uppercase tracking-wider">Debugger</span></div>
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            <div>
                ...
                <div className={`p-3 rounded-lg border ${isAiThinking ? 'bg-[#a8c7fa]/10 border-[#a8c7fa]/30 text-[#a8c7fa]' : 'bg-[#444746]/20 border-[#444746]/50 text-[#c4c7c5]'}`}><div className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${isAiThinking ? 'bg-[#a8c7fa] animate-pulse' : 'bg-[#66bb6a]'}`} /><span className="text-sm font-mono">{isAiThinking ? 'Processing Vision...' : 'Waiting for Input'}</span></div></div>
            </div>
            {debugInfo?.screenshotBase64 && (
                <div><div className="flex items-center gap-2 mb-2 text-[#c4c7c5] text-xs font-bold uppercase tracking-wider"><Eye className="w-3 h-3" /> Vision Input</div><div className="rounded-lg overflow-hidden border border-[#444746] bg-black/50"><img src={debugInfo.screenshotBase64} alt="AI Vision" className="w-full h-auto opacity-80" /></div></div>
            )}
        </div>
        <div className="p-3 bg-[#252525] border-t border-[#444746] text-center"><p className="text-[10px] text-gray-500 font-medium">Powered by Google Gemini 3 Flash</p></div>
      </div>
    </div>
  );
};
